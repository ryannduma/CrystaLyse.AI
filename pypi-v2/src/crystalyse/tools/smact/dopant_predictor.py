...
class DopantPredictionResult(BaseModel):
    ...
    # Dopant suggestions
    n_type_cation: List[DopantSuggestion] = Field(default_factory=list, description="N-type cation dopants")
    p_type_cation: List[DopantSuggestion] = Field(default_factory=list, description="P-type cation dopants")
    n_type_anion: List[DopantSuggestion] = Field(default_factory=list, description="N-type anion dopants")
    p_type_anion: List[DopantSuggestion] = Field(default_factory=list, description="P-type anion dopants")
...
class SMACTDopantPredictor:
    ...
    def predict_dopants(
        ...
        try:
            ...
            # Initialize Doper
            doper = Doper(
                original_species=tuple(species),
                embedding=embedding,
            ...
            def parse_dopant_list(dopant_data: List) -> List[DopantSuggestion]:
                ...
                for entry in dopant_data:
                    if len(entry) >= 6:  # Full data with selectivity
                        suggestions.append(DopantSuggestion(
            ...
            n_type_cat = parse_dopant_list(results.get("n-type cation substitutions", {}).get("sorted", []))
            p_type_cat = parse_dopant_list(results.get("p-type cation substitutions", {}).get("sorted", []))
            n_type_an = parse_dopant_list(results.get("n-type anion substitutions", {}).get("sorted", []))
            p_type_an = parse_dopant_list(results.get("p-type anion substitutions", {}).get("sorted", []))

...
from crystalyse.workspace.workspace_tools import ClarificationRequest, Question
...
class TestEnhancedErrorHandling:
    ...
    async def test_validation_error_handling(self):
        ...
        with pytest.raises(ToolValidationError):
            await tool_caller.call_with_retry(
                validation_error_function, tool_name="test_tool", max_retries=1
            )
    ...
    async def test_graceful_degradation(self):
        ...
        with pytest.raises(ToolDegradedError) as exc_info:
            await tool_caller.call_with_fallback(
                failing_primary, working_fallback, tool_name="test_tool"
            )
...
class TestIntegrationWorkflow:
    ...
    async def test_complete_clarification_workflow(self):
        ...
        questions = [
            ...
            Question(
                id="chemistry", text="Which chemistry?", options=["Li-ion", "Na-ion", "K-ion"]
            ),
        ]

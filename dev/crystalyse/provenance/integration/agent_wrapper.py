...
class CrystaLyseWithProvenance:
    ...
    def __init__(
        ...
        # Initialize agent
        self.agent = EnhancedCrystaLyseAgent(mode=mode, project_name=self.project_name)

        # Track sessions
        self.sessions = []

    async def discover(
        self, query: str, session_id: str | None = None, timeout: int | None = None
    ) -> dict[str, Any]:
        """
        ...
        if trace_handler.event_logger:
            trace_handler.event_logger.log(
                "discovery_start",
                {
                    "query": query,
                    "mode": self.mode,
        ...
        try:
            ...
            result = await asyncio.wait_for(
                self.agent.discover(query, trace_handler=trace_handler), timeout=timeout
            )
            ...
            if trace_handler.event_logger:
                trace_handler.event_logger.log(
                    "discovery_timeout",
                    {"timeout_s": timeout, "timestamp": datetime.now().isoformat()},
                )
            ...
            if trace_handler.event_logger:
                trace_handler.event_logger.log(
                    "discovery_error", {"error": str(e), "timestamp": datetime.now().isoformat()}
                )
    ...
    def get_materials_catalog(self, session_id: str) -> list | None:
    ...
    def get_provenance_events(self, session_id: str) -> list | None:

    ...
    for mode in modes:
        try:
            ...
            if result["status"] == "completed":
                console.print(
                    f"[green]✓ {mode}: {result['materials_found']} materials, "
            ...
            results.append({"mode": mode, "status": "error", "error": str(e)})
        ...
        if result.get("status") == "completed":
            ...
            if missing:
                ...
                if result.get("energy_capture_rate", 0) < 100:
                    console.print(
                        f"  [yellow]⚠ {mode}: Only {result['energy_capture_rate']:.0f}% materials have energies[/yellow]"
                    )
                else:
                    console.print(
                        f"  [green]✓ {mode}: Complete provenance with all energies[/green]"
                    )

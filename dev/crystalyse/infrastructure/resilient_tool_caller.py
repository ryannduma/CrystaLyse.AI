...
class ResilientToolCaller:
    ...
    async def call_with_retry(
        ...
        **kwargs,
        ...
        for attempt in range(max_retries):
            try:
                logger.info(
                    f"🔧 Calling {tool_name} ({operation_type}) - attempt {attempt + 1}/{max_retries}"
                )
                ...
                result = await asyncio.wait_for(tool_func(*args, **kwargs), timeout=timeout)
                ...
                logger.info(
                    f"✅ {tool_name} call succeeded in {duration:.1f}s (attempt {attempt + 1})"
                )
                return result
        ...
        logger.error(
            f"📊 Stats: {stats['successful_calls']}/{stats['total_calls']} success rate, avg duration: {stats['avg_duration']:.1f}s"
        )
        ...
        **kwargs,
        ...
        try:
            return await self.call_with_retry(
                primary_func, *args, tool_name=tool_name, operation_type=operation_type, **kwargs
            )
    ...
    def get_timeout(self, tool_name: str, operation_type: str, context: dict | None = None) -> int:
        ...
        # Get base timeout
        timeout = base_timeouts.get(
            tool_name, base_timeouts.get(operation_type, base_timeouts["default"])
        )
...
_resilient_caller: ResilientToolCaller | None = None

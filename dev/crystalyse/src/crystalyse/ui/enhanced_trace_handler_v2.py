...
class EnhancedToolCall:
    ...
    mcp_tool: Optional[str] = None  # Actual MCP tool name
...
class EnhancedTraceHandlerV2(ToolTraceHandler):
    ...
    def _detect_mcp_tool(self, output: Any) -> Optional[str]:
        ...
        try:
            ...
            if self.event_logger:
                self.event_logger.log("parse_error", {
                    "error": str(e),
    ...
    def _extract_materials_comprehensive(self, output: Any, tool_name: Optional[str]) -> List[Dict]:
    ...
    def finalize(self):
        ...
        summary = {
            ...
            "unique_compositions": len(set(m["composition"] for m in self.materials_found)),
        ...
        if self.materials_found:
            energies = [m["formation_energy"] for m in self.materials_found if m.get("formation_energy")]

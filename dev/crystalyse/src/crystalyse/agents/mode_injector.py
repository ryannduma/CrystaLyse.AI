        ...
        if self.tool_name in self.mode_required_tools or "analysis" in self.tool_name.lower():
            ...
            if "mode" in kwargs:
                ...
                if original_mode != self.current_mode:
                    logger.info(f"Mode injection: Overriding agent mode '{original_mode}' with current mode '{self.current_mode}' for {self.tool_name}")
            else:
                logger.info(f"Mode injection: Adding mode '{self.current_mode}' to {self.tool_name}")
...
def inject_mode_into_mcp_servers(mcp_servers: List[Any], current_mode: str) -> List[Any]:
    ...
    logger.info(f"Mode injection configured for {len(mcp_servers)} MCP servers with mode='{current_mode}'")
...
class DynamicModeSuppressor:
    ...
    def log_suppressed_switch(attempted_mode: str, reason: str):
        ...
        logger.info(f"Dynamic mode switch suppressed: {current_mode} -> {attempted_mode} (reason: {reason}) - Mode is locked")
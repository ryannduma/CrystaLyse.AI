...
def analyze(
    ...
    if summary:
        ...
        if mat_summary.get("avg_energy"):
            console.print(f"  Energy range: {mat_summary['min_energy']:.3f} to {mat_summary['max_energy']:.3f} eV/atom")
...
def compare(
    ...
    # Compare materials
    mats1 = set(m["composition"] for m in data1["materials"])
    mats2 = set(m["composition"] for m in data2["materials"])

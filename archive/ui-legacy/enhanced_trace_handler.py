...
class EnhancedTraceHandler(ToolTraceHandler):
    ...
    def _on_tool_call_start_provenance(self, item):
        ...
        tool_call = ToolCall(
            tool_name=tool_name, args=tool_args, start_time=time.time(), call_id=call_id
        )
    ...
    def _on_message_output_provenance(self, item):
        ...
        if not self.first_token_time:
            ...
            self.event_logger.log("ttfb", {"time_ms": ttfb, "ts": datetime.now().isoformat()})
    ...
    def _on_reasoning_provenance(self, item):
        ...
        if hasattr(item, "content") and item.content:
            ...
            self.event_logger.log(
                "reasoning", {"length": len(item.content), "ts": datetime.now().isoformat()}
            )
    ...
    def _extract_provenance_from_output(self, tool_name: str, output: Any):
        ...
        try:
            ...
            self.event_logger.log("extraction_error", {"tool": tool_name, "error": str(e)})
    ...
    def finalize(self):
        ...
        for tc in self.tool_calls.values():
            if tc.tool_name not in tool_stats:
                tool_stats[tc.tool_name] = {"count": 0, "total_ms": 0, "avg_ms": 0}
            tool_stats[tc.tool_name]["count"] += 1
            tool_stats[tc.tool_name]["total_ms"] += tc.duration_ms

        for _tool_name, stats in tool_stats.items():
    ...
    def write_with_tracking(file_path: str, content: str) -> str:
        ...
        if is_cif:
            try:
                ...
                sorted_sites = sorted(
                    struct.sites, key=lambda s: (s.species_string, tuple(s.frac_coords))
                )
